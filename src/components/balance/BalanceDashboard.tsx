...
// ── Chain Icons ────────────────────────────────────────────
// Files in /public/chains/{id}.png — fallback to colored dot
  ...
  if (src) {
    return (
      <img
        ...
        onError={(e) => {
          // fallback to colored dot if file missing
...
const CHAIN_BALANCES: ChainBalance[] = [
  { id: "arc",  name: "Arc Testnet",      short: "Arc",  color: "#C8102E", available: 450.00, pending: 0,     wallet: 1250.00 },
...
export default function BalanceDashboard() {
  ...
  const totalUnified = CHAIN_BALANCES.reduce((s, c) => s + c.available, 0);
  const totalPending  = CHAIN_BALANCES.reduce((s, c) => s + c.pending,   0);
  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-4">
      ...
      <div className="overflow-hidden rounded-card2 border border-ink-border bg-ink-surface shadow-card">
        ...
        <div className="p-5 sm:p-6">
          ...
          <p className="mt-3 font-body text-sm text-muted">
            Aggregated across <span className="text-cream-dim">{CHAIN_BALANCES.length} chains</span> via Circle Gateway.
          </p>
        </div>
      </div>
      ...
      {/* ── Per-chain breakdown ──────────────────────────── */}
      <div className="rounded-card2 border border-ink-border bg-ink-surface p-4 shadow-card sm:p-5">
        {/* Header — hide balance labels on mobile (shown inline instead) */}
              ...
              <div className="rounded-card border border-ink-border2 bg-ink-surface2 p-3">
                ...
                <div className="hidden items-center gap-3 sm:flex">
                  ...
                  <div className="flex flex-1 justify-end gap-6 font-mono text-sm">
                    <span className="text-cream-white">${chain.available.toFixed(2)}</span>
                    <span className={chain.pending > 0 ? "text-yellow-400" : "text-muted"}>
                      ${chain.pending.toFixed(2)}
                    </span>
                    <span className="text-muted">${chain.wallet.toFixed(2)}</span>
                  </div>
